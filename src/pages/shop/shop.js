...
import {shopData} from "../../assets/data/food";
var url = require('url');
class Shop extends Component {
  ...
  componentDidMount () {
    ...
    // url.parse(httpUrl, true).query.name;
    this.initData(url.parse(httpUrl, true).query.shopInfo);
  }
    ...
    return (
      <div className="shop-container">
          ...
          <div className="description-header">
            <div className="description-top">
              <div className="description-left">
                <img src={imgUrl + this.state.shopInfo.picUrl} alt=""/>
              </div>
              <div className="description-right">
                <div>{this.state.shopInfo.deliveryTimeTip} | {this.state.shopInfo.distance}</div>
                <div>{this.state.shopInfo.address}</div>
                <div className='slide-show'>
                  <div className='slide-content'>
                    {
                      this.state.shopInfo.discounts2 ? this.state.shopInfo.discounts2.map((ite, index) => {
                        return (
                            <div className='discount' key={index}>
                              <img className='dis-img' src={imgUrl + ite.iconUrl} alt='' />
        ...
        <div className='change-show-type'>
          <div><span onClick={this.changeShowType.bind(this, 'food')} className={this.state.active==='food'?'activity-show':''}>点菜</span></div>
          <div><span onClick={this.changeShowType.bind(this, 'rating')} className={this.state.active==='rating'?'activity-show':''}>评价</span></div>
          <div><span onClick={this.changeShowType.bind(this, 'food')} className={this.state.active==='restaurant'?'activity-show':''}>商家</span></div>
        </div>
                                      ...
                                      <div className='menu-detail-list' key={foodIndex}>
                                        <Link to='/shop/foodDetail' className='menu-detail-link'>
                                          <div className='menu-food-img'>
                                            <img src={`${imgUrl}food${Math.floor(Math.random()*7)}.png`} alt=""/>
                                          </div>
                                          <div className='menu-food-description'>
                                            <h3 className="food-description-head">
                                              ...
                                              {food.attributes.length?<ul className='attributes-ul'></ul>:''}
                                            </h3>
                                            <div className="food-description-content">{food.description}</div>
                                            <div className='food-description-sale-rating'>
                ...
                <div className="buy-cart-container">
                  <div className='cart-icon-num'>
                    <div onClick={this.handleShowCart} className={this.state.count===0?"cart-icon-container":this.state.animate}>
                      <img className='img-cart' src={imgUrl+'cart.png'} alt=''/>
